...
//그러나 그 위치를 모르면 저장된 값을 가져갈 수 없으므로,자료의 저장 위치를 나타내는 이름이 필요하다. 
//그것을 변수명이라고 한다.
public class Va01 {
	...
	public static void main(String[] args) {
		 ...
		 * 영문 대문자와 소문자로 된 뵨수명은 서로 다르게 인식된다.
		 * 변수명 중간에는 공백과 $,_  제외한 특수문자는 지정할 수 없다.
		 * 상수인 경우는 일반적으로 모두 대문자로 지정한다.
		 * 
		 * 관례
		 * 누구나 저장된 데이터나 역할이 유추할 수 있도록 이름을 저장한다.
		 * 여러 단어를 합성한 변수명은 첫 번째 단어는 모두 소문자로 하고,
		 * 두 번째 단어의 첫 번째 글자는 대문자로 지정한다. -> 카멜방식 
		 *  	classEngPoint 낙타 봉 모양
